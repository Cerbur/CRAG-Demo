 ...
 *
 * <p>定时扫描 chunk 表中需要做 Sparse FTS 分词的 child chunk（INIT / FAILED / 超时 PROCESSING）， 通过 CAS
 * 抢占方式保证多实例并发安全，直接调用 ChunkFtsDao 写入 tsvector 全文检索记录.
 *
 * <p>与 DenseEmbeddingCron 的核心差异： - Sparse 不调外部 HTTP 服务，直接在 DB 侧完成 CJK 分词 + tsvector 转换 - 无
 * EmbeddingException，异常来源只有 DB 写入冲突
 *
 * <p>职责边界： - 本类负责定时触发、流程编排（只依赖 Dao，不直接依赖 Repository） - ChunkDao 负责 chunk 表读写 + CAS 抢占 - ChunkFtsDao
 * 负责 chunk_fts 表读写 + 幂等检查
 *
 * @since 2026-06-14
 */
@Component
public class SparseEmbeddingCron {
   ...
   * FAILED→PROCESSING, T3: 超时 PROCESSING→PROCESSING） 3. 抢占成功后先做幂等检查，FTS 已存在则直接标记 SUCCESS 4. 写入
   * FTS（chunkFtsDao.insert，内部含 CJK 分词 + tsvector 转换） 5. 成功 → 标记 SUCCESS（T4） 6. 失败 → 标记
   * FAILED（T5），下轮自动重试
   */
  @Scheduled(cron = "*/10 * * * * *")
  void processSparseEmbedding() {
    ...
    for (Chunk chunk : candidates) {
      ...
      try {
        ...
        log.warn(
            "Sparse FTS duplicate for chunk {}, will retry next round: {}",
            chunk.getChunkId(),
            e.getMessage());
        try {
        ...
        log.error(
            "Unexpected error during sparse FTS for chunk {}, marking FAILED",
            chunk.getChunkId(),
            e);
        try {
          chunkDao.updateSparseStatus(chunk.getChunkId(), ChunkStatus.FAILED, chunk.getVersion());
    ...
    log.info(
        "SparseEmbeddingCron round complete — success={}, failed={}, skipped={}",
        successCount,
        failedCount,
        skippedCount);
  }
