...
| 编号 | 任务 | 状态 | 提交 | 完成时间 |
| --- | --- | --- | --- | --- |
| 8.hotfix_2.1 | 以测试先行升级 v3 校验规则 | ✅ 完成 | 823df0a, a2c4ce8, ca347fb | 2026-06-20 |
...
| 2026-06-20 | 独立验收 session / Python 3 | `python3 -m unittest scripts.tests.test_validate_plans -v` | 通过 | fresh 复跑 20/20 通过；覆盖待验收、废弃前置与执行队列逆序 |
| 2026-06-20 | 独立验收 session / Git repository | `python3 scripts/validate_plans.py --strict --verify-git`、残留关键词检索、`git diff --check` | 通过 | 0 error；24 条兼容历史 warning 已核对；无 v2/owner/pending 规则残留或 whitespace error |
| 2026-06-20 | 独立验收 session / Gradle 9.4.1 / Java 21 | `./gradlew check` | 通过 | 沙箱内文件锁通信受限；沙箱外 `BUILD SUCCESSFUL in 1s`，51 项任务中 3 项执行、48 项缓存命中 |
| 2026-06-20 | 独立验收 session / Git commits | `git show --stat 823df0a a2c4ce8 ca347fb` | 通过 | 实现提交均属于本 Hotfix 文件边界；统一依赖不变量满足前置完成或执行队列有序的规则 |
...
| 2026-06-20 | 独立验收通过并完成 Hotfix | 统一依赖状态不变量、全部任务证据和仓库级完成门槛均已复核通过 | 4 项任务全部完成；workflow v3 正式生效并移出验收队列 |
