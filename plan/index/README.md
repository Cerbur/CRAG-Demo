...
> 最后更新：2026-06-30（plan_7.hotfix_1 独立验收通过并完成：success/failure 脚本迁移到 Knowledge 上传→ingestion READY→显式 KB Query；执行期发现 schema.sql 冷启动重置使「同 evidence 跨重建复用」不可行，7.hotfix_1.2 改为按目标 Stub 模式重建后 re-seed）
