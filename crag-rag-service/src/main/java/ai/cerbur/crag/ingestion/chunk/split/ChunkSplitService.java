 ...
 *
 * <p>分块策略： - parent chunk：大窗口（~1024 token），保留完整上下文，不做向量化 - child chunk：小粒度（~256 token 净新增），用于
 * Embedding + 检索匹配 - child chunk 之间有 overlap（~64 token），实际单 chunk 上限 ~320 token
 *
 * <p>Token 计数使用 JTokkit CL100K_BASE 编码，与 DeepSeek / GPT-4 系列 tokenizer 一致. 分块边界由 TokenTextSplitter
 * 按句末标点（.?!\n）自动选择，语义完整性优于固定窗口.
 *
 * @since 2026-06-10
 */
...
public class ChunkSplitService {
  ...
  /**
   * TokenTextSplitter 最小可嵌入长度. 设为 0 确保任何非空文本都不会被 TokenTextSplitter 静默丢弃. 默认值 5 会导致 <= 5 字符的 chunk
   * 丢失. 注意：TokenTextSplitter 使用严格大于（>）比较，0 即 length > 0 通过.
   */
   ...
   *
   * <p>Parent 级使用 TokenTextSplitter(chunkSize=1024) 切分全部 parent chunks； 在每个 parent 内部使用
   * TokenTextSplitter(chunkSize=256) 切分 child， child 之间通过 JTokkit 添加 ~64 token 重叠.
   *
   * @param content 原始纯文本
  ...
  public ChunkSplitResult split(String content) {
    if (content == null || content.isEmpty()) {
      ...
      return new ChunkSplitResult(new ChunkSplitData("", 0, null), Collections.emptyList());
    }
   ...
   *
   * <p>用于 child chunk 之间的 overlap：取前一个 child 的最后 N 个 token， 拼接到下一个 child 开头，防止语义边界截断.
   *
   * @param text 输入文本
