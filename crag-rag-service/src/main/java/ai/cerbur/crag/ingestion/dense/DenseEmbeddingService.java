 ...
 *
 * <p>入库阶段：由 Cron 调用，对 child chunk 做向量化后写入 pgvector. 检索阶段：将用户 query 向量化后送入 DenseQueryService（后续 plan
 * 实现）.
 *
 * <p>本类只负责核心 Embedding 调用逻辑和异常转换，流程编排由 cron/DenseEmbeddingCron 负责.
 *
 * @since 2026-06-10
 */
@Component
public class DenseEmbeddingService {
   ...
   * 对文本做向量化.
   *
   * <p>直接委托 EmbeddingClient，EmbeddingException 向上传播给 Cron 做终态处理.
   *
   * @param text 输入文本
