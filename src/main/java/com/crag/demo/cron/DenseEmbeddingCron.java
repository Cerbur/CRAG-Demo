package com.crag.demo.cron;

import com.crag.demo.core.dense.DenseEmbeddingService;
 ...
 *
 * 定时扫描 chunk 表中需要做 Dense Embedding 的 child chunk（INIT / FAILED / 超时 PROCESSING），
 * 通过 CAS 抢占方式保证多实例并发安全，调用 DenseEmbeddingService 做核心向量化，
 * 最后通过 ChunkEmbeddingDao 写入向量并更新 chunk 状态.
 *
 * 职责边界：
 * - 本类负责定时触发、流程编排（只依赖 Dao，不直接依赖 Repository）
 * - ChunkDao 负责 chunk 表读写 + CAS 抢占
 * - DenseEmbeddingService 负责核心 Embedding 调用逻辑
 * - ChunkEmbeddingDao 负责 chunk_embedding 表读写 + pgvector 类型转换
 * - 后续新增其他定时任务同样放入 cron 包
 *
 * @since 2026-06-13
 */
@Component
public class DenseEmbeddingCron {
     ...
     * 3. 抢占成功后调 DenseEmbeddingService.embed() 做向量化
     * 4. 成功 → 幂等检查（chunk_embedding 是否已存在）→ 不存在则 INSERT → 标记 SUCCESS（T4）
     * 5. 失败 → 标记 FAILED（T5），下轮自动重试
     */
    @Scheduled(cron = "*/10 * * * * *")
    void processDenseEmbedding() {
        ...
        List<Chunk> candidates = chunkDao.findDenseCandidates(
        ...
        for (Chunk chunk : candidates) {
            ...
            int affected = switch (chunk.getDenseStatus()) {
                case INIT -> chunkDao.tryMarkProcessing(chunk.getChunkId(), ChunkStatus.INIT,
                    chunk.getVersion());                                                                       // T1
                case FAILED -> chunkDao.tryMarkProcessing(chunk.getChunkId(), ChunkStatus.FAILED,
                    chunk.getVersion());                                                                       // T2
                case PROCESSING -> chunkDao.tryMarkProcessingTimeout(chunk.getChunkId(),
        ...
        log.info("DenseEmbeddingCron round complete — success={}, failed={}, skipped={}",
