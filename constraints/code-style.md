...
- 非必要不要新增依赖构造器，保持项目内注入方式一致。

### 例外

框架、配置工厂、不可变值对象、必须脱离 Spring 手工构造的对象，或存在明确测试与设计收益时，可以使用构造器注入。偏离默认方式时应能说明与当前需求直接相关的理由。

### 迁移

修改既有 Spring 组件时，不得仅为风格偏好把 `@Autowired` 字段注入迁移为构造器注入。已存在的非必要构造器注入在相关任务触及时恢复为默认方式，不要求无关任务全量重构。
...
纯单元测试为验证隔离的 Java 并发逻辑，可创建局部执行器，但必须可靠关闭。第三方库内部线程不直接受本规则约束，但应优先配置其线程池与关闭行为。

---

## 十二、测试代码风格

测试分类、命名、执行边界与覆盖要求见 `constraints/test-workflow.md`。本节只约束测试代码写法，不以“测试代码”统称掩盖纯单元、轻量组件、架构或 Docker HTTP 回归之间的环境差异。
...
- 纯单元测试不得依赖执行顺序、真实网络、数据库、文件系统或系统环境状态。
- 轻量组件测试使用 Spring Context、Spring Slice、Mock HTTP Client 或 H2 时，文件与类名必须使用 `*ComponentTest`，且不得把替身环境结果表述为真实 PostgreSQL、pgvector、Sidecar 或端到端保证。
- 架构测试文件与类名必须使用 `*ArchitectureTest`，只验证包、模块和依赖规则，不承载业务流程断言。
