 ...
 * /actuator/health/readiness 返回 HTTP 200 且 status=UP，响应不含 components 详情；
 * /actuator/env 等管理端点不暴露。
 *
 * <p>H2 仅为轻量组件测试替身。本测试可证明 Bean 装配、配置绑定和受控替身下的基础组件协作，
 * 不能证明 PostgreSQL 方言、native SQL、JSONB、pgvector、锁、CAS、真实事务隔离、
 * 容器网络或 Sidecar 协议正确。真实持久化与端到端行为必须通过 Docker HTTP 回归验证。
 *
