 ...
 *
 * 位于检索管道中间层：消费 {@link SparseSearchResult} + {@link DenseSearchResult}，
 * 产出包含 RRF 融合分数及最佳 child 原始得分的窄类型.
 * 下游 Rerank 阶段消费此类型并组装为最终的宽类型 {@link ChunkSearchResult}.
 *
 * @since 2026-06-17
 */
public class RrfFusionResult {
    ...
    public RrfFusionResult(String chunkId, double rrfScore, String content,
                           Double bestSparseScore, Double bestDenseScore) {
        this(chunkId, null, null, rrfScore, content, bestSparseScore, bestDenseScore);
    }

    public RrfFusionResult(String chunkId, String parentChunkId, Integer chunkIndex,
                           double rrfScore, String content,
                           Double bestSparseScore, Double bestDenseScore) {
        this(new ChunkBO(chunkId, parentChunkId, chunkIndex, content), rrfScore, bestSparseScore, bestDenseScore);
    }

    public RrfFusionResult(ChunkBO chunk, double rrfScore, Double bestSparseScore, Double bestDenseScore) {
        this.chunk = chunk;
