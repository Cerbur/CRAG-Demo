...
/**
 * 重排序服务 —— 消费 {@link RrfFusionResult} 列表，委托 RerankClient 做语义重排序，
 * 通过 {@link ChunkSearchResult#fromRrfWithRerank(RrfFusionResult, double)} 组装最终宽类型.
 *
 * Rerank 是检索模块内部细节，在 RetrievalService 内部完成，
 * 外部模块（如 crag-query）不需要感知此步骤.
 *
 * @since 2026-06-10
 */
@Component
public class RerankService {
     ...
     *
     * 提取每个 chunk 的内容文本，委托 RerankClient 获取 rerank 分数，
     * 通过 {@link ChunkSearchResult#fromRrfWithRerank(RrfFusionResult, double)}
     * 组装包含全部四路得分的最终宽类型，按 rerank score 降序排列.
     *
     * @param query  用户问题
    ...
    public List<ChunkSearchResult> rerank(String query, List<RrfFusionResult> chunks) {
        ...
        List<String> documents = chunks.stream()
            .map(RrfFusionResult::getContent)
            .toList();
        ...
        if (log.isDebugEnabled()) {
            for (int i = 0; i < Math.min(reordered.size(), 10); i++) {
                ...
                log.debug("  rerank[{}] chunk={} sparse={} dense={} rrf={} rerank={}",
                    i, r.getChunkId(), r.getSparseScore(), r.getDenseScore(),
                    r.getRrfScore(), r.getRerankScore());
            }
        }
