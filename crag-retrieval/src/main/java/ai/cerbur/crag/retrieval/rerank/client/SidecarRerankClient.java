 ...
 *
 * <p>协议：POST /rerank，body: {"query": "...", "documents": [...]} → {"results": [{"index": 0,
 * "score": 0.95}, ...]}. 模型：bge-reranker-v2-m3 (CrossEncoder). 异常时记录日志并返回空结果，由上层 RerankService
 * 降级回退.
 *
 * @since 2026-06-15
 */
...
public class SidecarRerankClient implements RerankClient {
  ...
  void init() {
    ...
    log.info(
        "SidecarRerankClient initialized — baseUrl={}, connectTimeout={}, readTimeout={}",
        sidecarUrl,
        connectTimeout,
        readTimeout);
  }
  ...
  public List<RerankResult> rerank(String query, List<String> documents) {
    ...
    log.debug(
        "Sending rerank request — query length={}, document count={}",
        query.length(),
        documents.size());
