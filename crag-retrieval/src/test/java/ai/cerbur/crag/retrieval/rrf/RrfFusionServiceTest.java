 ...
 *
 * RRF 阶段不回表 parent、不做 parent 去重；相邻 child 扩展交由 RetrievalService 处理.
 *
 * @since 2026-06-15
 */
...
class RrfFusionServiceTest {
    ...
    private SparseSearchResult sparse(String chunkId, String parentChunkId, double score, String content) {
        return new SparseSearchResult(chunkId, parentChunkId, score, content);
    }

    private DenseSearchResult dense(String chunkId, String parentChunkId, double score, String content) {
    ...
    class EmptyInputProtection {
        ...
        void bothEmptyReturnsEmpty() {
            List<RrfFusionResult> results = rrfFusionService.fuse(
                Collections.emptyList(), Collections.emptyList(), TOP_N);
        ...
        void sparseEmptyDenseValidReturnsChildChunk() {
            List<DenseSearchResult> denseList = List.of(
                new DenseSearchResult("c1", "p1", 4, 0.95, "子内容一")
            );

            List<RrfFusionResult> results = rrfFusionService.fuse(
                Collections.emptyList(), denseList, TOP_N);
    ...
    class RrfScoreCalculation {
        ...
        void singleStreamSingleResultCorrectScore() {
            List<SparseSearchResult> sparseList = List.of(
                sparse("c1", "p1", 0.99, "内容")
            );

            List<RrfFusionResult> results = rrfFusionService.fuse(
                sparseList, Collections.emptyList(), TOP_N);
    ...
    class ChildGranularity {
        ...
        void multipleChildrenUnderSameParentAreKept() {
            List<SparseSearchResult> sparseList = List.of(
                sparse("c1", "p1", 0.90, "子1"),
                sparse("c2", "p1", 0.80, "子2")
            );
    ...
    class SortingAndLimit {
        ...
        void resultsSortedByRrfScoreDescending() {
            ...
            assertThat(results).extracting(RrfFusionResult::getChunkId)
        ...
        void topNLimitApplied() {
            ...
            List<RrfFusionResult> results = rrfFusionService.fuse(
                sparseList, Collections.emptyList(), 2);
